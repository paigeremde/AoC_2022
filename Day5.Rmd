...
List2 <- lapply(Day5input, function(column) as.list(na.omit(column)))

for (i in seq_along(Instructions$N)) {
  NCrates <- Instructions$N[i]
  From <- Instructions$`F`[i]
  To <- Instructions$`T`[i]
  Values <- tail(List2[[From]], n = NCrates)
  List2[[To]] <- append(List2[[To]], values = Values, after = length(List2[[To]]))
  List2[[From]] <- head(List2[[From]], -NCrates)
}

TopCrates2 <- as.character(sapply(List2, function(lst) tail(lst, n = 1)))
